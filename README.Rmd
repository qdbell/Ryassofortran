...
The `calibrate_yasso()` function is highly specialized and not intended for standard use. It is utilized for model calibration at the Finnish Meteorological Institute: In the database used for calibration, there is a measured initial state and an observed carbon value at each time step. Thus, the initial carbon is passed to the function as a matrix and the model uses a pre-defined initial state at each time step. Furthermore, the leaching input is a singular value instead of a vector, since every calibration dataset has a characteristic leaching.
